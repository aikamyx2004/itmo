...
public class FrontServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        sourceConfiguration = newFreemarkerConfiguration(
                getServletContext().getRealPath("/") + "../../src/main/webapp/WEB-INF/templates", false);
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; clazz != null; clazz = clazz.getSuperclass()) {
            List<Method> currentMethod = Arrays.stream(clazz.getDeclaredMethods())
                    .filter(m -> Objects.equals(m.getName(), route.getAction()))
                    .toList();
            if (currentMethod.size() >= 2) {
                response.sendRedirect("/not/found");
                throw new ServletException("many such methods");
            }
        ...
        try {
            ...
            for (Class<?> clazz : method.getParameterTypes()) {
                if (Map.class.equals(clazz)) {
                    params.add(view);
                } else if (HttpServletRequest.class.equals(clazz)) {
                    params.add(request);
                }
            }
        ...
        Template template;
        if (lang != null) {
            template = newTemplate(pageClass.getSimpleName() + ".ftlh", new Locale(lang));
        } else {
            template = newTemplate(pageClass.getSimpleName() + ".ftlh", null);
        }
