...
public class EventRepositoryImpl implements EventRepository {
    ...
    public void save(Event event) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            ...
            throw new RepositoryException("Could not save event: " + event.getType(), e);
        }
    }
    ...
    private Event find(long id) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            ...
            throw new RepositoryException("Could not find event in database", e);
        }
    }
        ...
        for (int i = 1; i <= metaData.getColumnCount(); i++) {
            switch (metaData.getColumnName(i)) {
                ...
                case "userId":
                    ...
                    break;
                case "type":
                    event.setType("ENTER".equals(resultSet.getString(i)) ? EventType.ENTER : EventType.LOGOUT);
                    break;
                case "creationTime":
                    event.setCreationTime(resultSet.getTimestamp(i));
                    break;
                default:
                    throw new RepositoryException("Unexpected column in Event" + metaData.getColumnName(i) + ".");
            }
        }
        return event;
    }
