...
public class UserRepositoryImpl implements UserRepository {
    ...
    public User find(long id) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            ...
            throw new RepositoryException("Could not take user count", e);
        }
    }
    ...
    public User findByLoginOrEmailAndPasswordSha(String loginOrEmail, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM User WHERE " +
                    (loginOrEmail.contains("@") ? "email=?" : "login=?") + " AND passwordSha=?")
            ) {
        ...
        for (int i = 1; i <= metaData.getColumnCount(); i++) {
            switch (metaData.getColumnName(i)) {
                ...
                case "creationTime":
                    ...
                    break;
                default:
                    throw new RepositoryException("Unexpected column " + metaData.getColumnName(i) + ".");
            }
        }
